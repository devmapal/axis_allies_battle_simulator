...
public class LandDefenderHitOrderFragment extends HitOrderFragment {
    ...
    private void restore_state_from_preference() {
        ...
        ArrayList<String> defender_land_hit_order = null;
        String d_land_hit_order_str = null;
        try {
            d_land_hit_order_str = sharedPref.getString("d_land_hit_order", defaultJSON);
            Log.e(Constants.LOG, sharedPref.getAll().toString());
            JSONArray d_land_hit_order_json = new JSONArray(d_land_hit_order_str);
            list = new ArrayList<String>();
            for (int i = 0; i < d_land_hit_order_json.length(); i++) {
                list.add(d_land_hit_order_json.getString(i));
            }
        } catch (JSONException e) {
            if (BuildConfig.DEBUG) {
                Log.e(Constants.LOG,
                        String.format("Error loading defender land hit order from shared preferences, was [%s]", d_land_hit_order_str));
            }
        }
    }
    ...
    private void save_state_to_preference() {
        ...
        JSONArray d_land_hit_order_json = new JSONArray(list);
        String d_land_hit_order_str = d_land_hit_order_json.toString();
        editor.putString("d_land_hit_order", d_land_hit_order_str);
    ...
    public interface Constants {
        String LOG = "foss.devmapal.axis_allies_calc.axis_allies_calc.LandDefenderHitOrderFragment";
    }
